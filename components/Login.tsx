        ...
        <div className="min-h-screen bg-isu-red flex items-center justify-center p-4">
            <div className="bg-white rounded-lg shadow-2xl w-full max-w-md p-8">
                ...
                <form onSubmit={handleChangePassword} className="space-y-4">
                    <div><label className="block text-sm font-medium text-gray-700 mb-1">新密碼</label><input type="password" className="w-full border border-gray-300 rounded-lg px-4 py-2 focus:ring-2 focus:ring-isu-red outline-none transition-all" value={newPass} onChange={e => setNewPass(e.target.value)} placeholder="請輸入新密碼" /></div>
                    <div><label className="block text-sm font-medium text-gray-700 mb-1">確認新密碼</label><input type="password" className="w-full border border-gray-300 rounded-lg px-4 py-2 focus:ring-2 focus:ring-isu-red outline-none transition-all" value={confirmPass} onChange={e => setConfirmPass(e.target.value)} placeholder="再次輸入新密碼" /></div>
                    {error && <div className="bg-red-50 text-red-600 p-3 rounded text-sm flex items-center gap-2"><ICONS.Alert size={16} /> {error}</div>}
                    <button type="submit" className="w-full bg-isu-dark text-white font-bold py-3 rounded-lg hover:bg-gray-800 transition-colors shadow-lg mt-4">確認變更並登入</button>
                </form>
            </div>
        </div>
      );
    ...
    <div className="min-h-screen bg-gradient-to-br from-isu-red to-red-900 flex items-center justify-center p-4">
      <div className="bg-white rounded-xl shadow-2xl w-full max-w-md overflow-hidden flex flex-col">
        ...
        <div className="flex border-b border-gray-200">
            <button className={`flex-1 py-4 text-sm font-medium transition-colors ${userType === 'STUDENT' ? 'border-b-2 border-isu-red text-isu-red bg-red-50' : 'text-gray-500 hover:bg-gray-50'}`} onClick={() => { setUserType('STUDENT'); setError(''); setAccount(''); setPassword(''); }}>學生登入</button>
            <button className={`flex-1 py-4 text-sm font-medium transition-colors ${userType === 'STAFF' ? 'border-b-2 border-isu-red text-isu-red bg-red-50' : 'text-gray-500 hover:bg-gray-50'}`} onClick={() => { setUserType('STAFF'); setError(''); setAccount(''); setPassword(''); }}>教職員登入</button>
        </div>
        <div className="p-8">
            <form onSubmit={handleLogin} className="space-y-5">
                ...
                <div>
                    ...
                    <div className="relative"><ICONS.Security className="absolute left-3 top-1/2 transform -translate-y-1/2 text-gray-400" size={18} /><input type="password" className="w-full pl-10 pr-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-isu-red focus:border-transparent outline-none transition-all" placeholder="請輸入密碼" value={password} onChange={e => setPassword(e.target.value)} /></div>
                    <div className="text-right mt-1"><a href="#" className="text-xs text-blue-600 hover:underline">忘記密碼?</a></div>
                </div>
                {error && <div className="bg-red-50 text-red-600 p-3 rounded text-sm flex items-center gap-2 animate-pulse"><ICONS.Alert size={16} /> {error}</div>}
                <button type="submit" className="w-full bg-isu-red text-white font-bold py-3 rounded-lg hover:bg-red-800 transition-colors shadow-md flex items-center justify-center gap-2"><ICONS.Login size={20} /> 登入系統</button>
            </form>
        </div>
