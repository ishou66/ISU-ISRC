    ...
    <div className="bg-white rounded-lg shadow-sm border border-gray-200 flex flex-col h-full">
      <div className="p-4 border-b border-gray-200 bg-gray-50 flex flex-wrap gap-4 items-center justify-between">
         <div className="flex items-center gap-2">
            <ICONS.Audit className="text-isu-dark" size={20} />
            <h2 className="text-lg font-bold text-gray-800">資安稽核日誌</h2>
         </div>
         ...
         <div className="flex gap-2">
            ...
            >
                ...
                <option value="VIEW_SENSITIVE">查看敏感資料</option>
                <option value="UPDATE">更新資料</option>
                <option value="CREATE">新增資料</option>
                <option value="ACCESS_DENIED">權限不足 (警示)</option>
                <option value="LOGIN">登入系統</option>
                <option value="SYSTEM_RESET">系統重置</option>
            </select>
         </div>
      </div>
      ...
      <div className="flex-1 overflow-auto">
        <table className="w-full text-sm text-left">
            <thead className="bg-gray-100 text-gray-700 sticky top-0">
                <tr>
                    <th className="px-4 py-2 w-32">時間戳記</th>
                    <th className="px-4 py-2 w-24">IP</th>
                    <th className="px-4 py-2 w-32">操作者 (角色)</th>
                    <th className="px-4 py-2 w-32">動作類型</th>
                    <th className="px-4 py-2">目標物件 / 詳情</th>
                ...
                {filteredLogs.map(log => (
                    <tr key={log.id} className={`border-b hover:bg-gray-50 ${log.status !== 'SUCCESS' ? 'bg-red-50 hover:bg-red-100' : ''}`}>
                        ...
                        <td className="px-4 py-2 font-medium">
                            {log.actorName} 
                            <span className="block text-[10px] text-gray-500">{log.roleName}</span>
                        </td>
                        <td className="px-4 py-2">
                            <span className="px-1 py-0.5 rounded border border-gray-200 bg-white">
                                {log.actionType}
                            </span>
                        </td>
                        <td className="px-4 py-2">
                            <div className="font-medium text-gray-800">{log.target}</div>
                            {log.details && <div className="text-gray-500 mt-1">{log.details}</div>}
                        </td>
                        <td className="px-4 py-2">
                            <span className={`px-2 py-0.5 rounded ${getStatusColor(log.status)}`}>
                                {log.status}
                            </span>
                        </td>
                    </tr>
