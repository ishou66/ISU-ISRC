...
interface UserManagerProps {
  ...
  onDeleteUser: (userId: string) => void; // Usually logical delete or deactivate
}

export const UserManager: React.FC<UserManagerProps> = ({ users, roles, onSaveUser, onDeleteUser }) => {
    ...
    <div className="bg-white rounded-lg shadow-sm border border-gray-200 flex flex-col h-full">
      <div className="p-4 border-b border-gray-200 bg-gray-50 flex justify-between items-center">
        <h2 className="text-lg font-bold text-gray-800 flex items-center gap-2">
            <ICONS.Users className="text-isu-dark" /> 使用者管理
        </h2>
        <div className="flex gap-2">
            ...
            <button 
                ...
                className="bg-isu-red text-white px-3 py-1.5 rounded text-sm hover:bg-red-800 flex items-center gap-2"
            >
                <ICONS.Plus size={16} /> 新增使用者
            </button>
        </div>
      </div>
        ...
        <table className="w-full text-sm text-left">
            <thead className="bg-gray-100 text-gray-700 sticky top-0">
                <tr>
                    ...
                    <th className="px-4 py-3">角色權限</th>
                ...
                {filteredUsers.map(user => (
                    <tr key={user.id} className="border-b hover:bg-gray-50">
                        <td className="px-4 py-3">
                            <span className={`px-2 py-1 rounded-full text-xs ${user.isActive ? 'bg-green-100 text-green-800' : 'bg-gray-200 text-gray-600'}`}>
                                {user.isActive ? '啟用' : '停用'}
                            </span>
                        </td>
                        ...
                        <td className="px-4 py-3 text-right flex justify-end gap-2">
                            <button 
                                ...
                                className="text-gray-500 hover:text-isu-dark p-1"
                            >
                                <ICONS.Edit size={16} />
                            </button>
                        </td>
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 z-50 flex items-center justify-center p-4">
            <div className="bg-white rounded-lg shadow-xl w-full max-w-md">
                <div className="p-4 border-b border-gray-200 flex justify-between items-center bg-gray-50">
                    <h3 className="font-bold text-gray-800">{editingUser.id ? '編輯使用者' : '新增使用者'}</h3>
                    <button onClick={() => setIsModalOpen(false)}><ICONS.Close size={20} /></button>
                </div>
                <div className="p-6 space-y-4">
                    <div className="grid grid-cols-2 gap-4">
                        <div>
                            <label className="block text-xs font-bold text-gray-500 mb-1">帳號</label>
                            <input 
                                className="w-full border rounded px-3 py-2"
                                value={editingUser.account || ''}
                                onChange={e => setEditingUser({...editingUser, account: e.target.value})}
                                disabled={!!editingUser.id} // Cannot change account after creation
                            />
                        </div>
                         <div>
                    ...
                    <div className="grid grid-cols-2 gap-4">
                        <div>
                            <label className="block text-xs font-bold text-gray-500 mb-1">單位</label>
                            <input 
                                className="w-full border rounded px-3 py-2"
                         ...
                         <div>
                            <label className="block text-xs font-bold text-gray-500 mb-1">角色</label>
                            <select 
                                className="w-full border rounded px-3 py-2"
                    ...
                    <div className="flex items-center gap-2 pt-2">
                        <input 
                            ...
                            className="w-4 h-4 text-isu-red focus:ring-isu-red"
                        />
                        <label htmlFor="isActive" className="text-sm text-gray-700">啟用此帳號</label>
                    </div>
                </div>
                <div className="p-4 border-t border-gray-200 flex justify-end gap-2 bg-gray-50">
                    <button onClick={() => setIsModalOpen(false)} className="px-4 py-2 border rounded hover:bg-gray-100 text-sm">取消</button>
                    <button onClick={handleSave} className="px-4 py-2 bg-isu-red text-white rounded hover:bg-red-800 text-sm">儲存</button>
                </div>
            </div>
        </div>
      )}
