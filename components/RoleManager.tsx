...
export const RoleManager: React.FC<RoleManagerProps> = ({ roles, onSaveRole, onDeleteRole }) => {
    ...
    <div className="flex h-full gap-6">
      ...
      <div className="w-1/4 bg-white rounded-lg shadow-sm border border-gray-200 flex flex-col">
        <div className="p-4 border-b border-gray-200 bg-gray-50 flex justify-between items-center">
            ...
            <button onClick={handleCreateClick} className="text-isu-red hover:bg-red-50 p-1 rounded">
                <ICONS.Plus size={18} />
            </button>
        </div>
            ...
            {roles.map(role => (
                <div 
                    ...
                    className={`p-3 rounded-lg cursor-pointer transition-colors border ${
                        ...
                        ? 'bg-isu-red text-white border-isu-red' 
                        : 'bg-white hover:bg-gray-50 border-gray-200 text-gray-700'
                    }`}
                >
                    <div className="font-bold">{role.name}</div>
                    <div className={`text-xs mt-1 truncate ${selectedRole?.id === role.id ? 'text-red-100' : 'text-gray-400'}`}>
                        {role.description}
                    </div>
                </div>
            ))}
            ...
            <>
                <div className="p-6 border-b border-gray-200 flex justify-between items-start">
                    <div className="flex-1">
                        {isEditing ? (
                            <div className="space-y-3">
                                <div>
                                    <label className="block text-xs font-bold text-gray-500">角色名稱</label>
                                    <input 
                                ...
                                <div>
                                    <label className="block text-xs font-bold text-gray-500">描述</label>
                                    <input 
                                        className="border border-gray-300 rounded px-2 py-1 w-full"
                    ...
                    <div className="flex gap-2">
                        {isEditing ? (
                            <>
                                <button onClick={() => setIsEditing(false)} className="px-3 py-1 text-gray-500 border rounded hover:bg-gray-50">取消</button>
                                <button onClick={handleSave} className="px-3 py-1 bg-isu-red text-white rounded hover:bg-red-800 flex items-center gap-1">
                                    <ICONS.Save size={14}/> 儲存
                                </button>
                            </>
                        ) : (
                            <>
                                {!selectedRole.isSystemDefault && (
                                    <button onClick={() => handleDelete(selectedRole.id)} className="px-3 py-1 text-red-600 border border-red-200 rounded hover:bg-red-50 flex items-center gap-1">
                                        <ICONS.Close size={14}/> 刪除
                                    </button>
                                )}
                                <button onClick={() => handleEditClick(selectedRole)} className="px-3 py-1 text-isu-dark border border-gray-300 rounded hover:bg-gray-50 flex items-center gap-1">
                                    <ICONS.Edit size={14}/> 編輯權限
                                </button>
                            </>
                        )}
                    </div>
                    ...
                    <table className="w-full text-sm text-left border-collapse">
                        <thead>
                            <tr className="bg-gray-100 border-b border-gray-200">
                                <th className="p-3 font-bold text-gray-700">功能模組</th>
                                <th className="p-3 text-center w-20">瀏覽</th>
                                <th className="p-3 text-center w-20">新增</th>
                                <th className="p-3 text-center w-20">編輯</th>
                                <th className="p-3 text-center w-20">刪除</th>
                                <th className="p-3 text-center w-20">匯出</th>
                                <th className="p-3 text-center w-20 bg-red-50 text-red-700">個資</th>
                            </tr>
                        </thead>
                        <tbody>
                                ...
                                return (
                                    <tr key={moduleId} className="border-b border-gray-100 hover:bg-gray-50">
                                        ...
                                        {(['view', 'add', 'edit', 'delete', 'export', 'viewSensitive'] as const).map(action => (
                                            <td key={action} className={`p-3 text-center ${action === 'viewSensitive' ? 'bg-red-50/30' : ''}`}>
                                                <input 
                                                    type="checkbox"
                                                    checked={perms[action]}
                                                    disabled={!isEditing}
                                                    onChange={() => handlePermissionToggle(moduleId, action)}
                                                    className="w-4 h-4 rounded text-isu-red focus:ring-isu-red disabled:opacity-50"
                                                />
                                            </td>
                                        ))}
                                    </tr>
